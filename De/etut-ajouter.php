...
if (isset($_POST['submit'])) {
  ...
  if ($nom == "" or $prenoms == "" or $telT == ""  or $email == "" or $login == "" or $password == "" or $mat == "" or $telP == "" or $photo == "") {
    echo "<script>alert('veillez remplir tous les champs');</script>";
  } else {
    $cls = mysqli_fetch_array(mysqli_query($info2->hackme(), "select CodeCLS from classe where LibelleCLS='$nomcl'"));
    $CodeCLS = $cls['CodeCLS'];
    // $fil=mysqli_fetch_array(mysqli_query($info2->hackme(),"select CodeCLS from classe where LibelleFIL='$nomcl'"));
    // $CodeFIL=$data['CodeFIL'];

    $ajout_et = $info2->ajouter_etudiant($mat, $nom, $prenoms, $telP, $telT, $email, $login, $password, $photo, $CodeCLS);
    if ($ajout_et == true) {
